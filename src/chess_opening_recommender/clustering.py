...
from scipy.spatial.distance import cdist  # Used for geometric median
...
def plot_points_in_clusters(clust_model: GaussianMixture,
    ...
    for x_i, x_val in enumerate(first_opening_vals):
        for y_i, y_val in enumerate(first_opening_vals):
            ...
            # Plot the graph
            plt.scatter(x_values, y_values, c=plot_colors, s=sizing, alpha=1)
...
def extract_openings(
    ...
    print("--- Initialisng data set and extracting first opening ---")
    first_opening_vals = read_opening_data(openings[0], color)
...
def create_clustering(
    ...
    clust_model = GaussianMixture(n_components=components,
                                  covariance_type=convariance_type, n_init=5)
...
# THE FOLLOWING METHODS ARE USED TO CALCULATE THE BEST OPENINGS FOR A REPERTOIRE
...
def find_fitting_openings(cluster_model: GaussianMixture,
                          ...
                          large_game_file: str,
                          color: bool,
                          recommendations: int = 10) -> List[Opening]:
    ...
    if not user_openings:
        return []
    large_openings = user_openings = dicts_to_openings(json_to_dicts(large_game_file))
...
def get_geometric_median(
        ...
        weights: List[int],
        epsillon: float = 1e-5) -> List[float]:
    ...
    # while True:
    #     distances = cdist(unpacked_data, [means])
    ...
    #     totals = np.sum(weights * unpacked_data[nonzeros], 0)

    #     num_zeros = len(unpacked_data) - np.sum(nonzeros)

    #     if num_zeros == 0:
    #         test_medians = totals

    #     elif num_zeros == len(unpacked_data):
    #         return means
...
def process_clustering(clust_file: str,
                       ...
                       clust_count: Union[int, None],
                       color: Union[bool, None]):
    """Call all relevent functions to recommend a set of openings for any or both
    colors"""

    if color is None:
        process_for_color(clust_file, user_file, export, clust_count, WHITE)
        process_for_color(clust_file, user_file, export, clust_count, BLACK)
    else:
        process_for_color(clust_file, user_file, export, clust_count, color)
...
def process_for_color(clust_file: str,
    ...
    if clust_count is None:
        clust_model = cluster_data(clust_file, color)
    ...
    if export_dest is not None:
        plot_points_in_clusters(clust_model, color, clust_file, export_dest)
        plot_points_in_clusters(clust_model, color, user_file, export_dest)
    ...
    for opening in openings:
        ...
        white_moves_split = list(opening.w_openings_count.keys())[0].split('|')[:-1]
        black_moves_split = list(opening.b_openings_count.keys())[0].split('|')[:-1]

        for w_move, b_move in zip(white_moves_split, black_moves_split):
            uci_notation += w_move + " " + b_move + " "

        opening.uci_opening = uci_notation

        out_dict["Recommended Openings"].append(opening.to_dict())

    with open(f'results/{base_file_name}_{color_str}.txt', "w", encoding='utf-8') as out:
        out.write(json.dumps(out_dict, indent=4))
    return

# cluster_testing("processed_DB_7.txt", BLACK)


def cluster_test(clust_file: str, data_file: str, color: bool):
    """Test file for clustering"""
    clust_model = cluster_data(clust_file, color)
    ...
    print("--- Finished Clustering Data ---")
    plot_points_in_clusters(clust_model, color, data_file)


if __name__ == "__main__":
    clu_f = input("What is the file to build the clusters from: ")
    gra_f = input("What is the data to display on the graphs: ")

    cluster_test(clu_f, gra_f, WHITE)
    cluster_test(clu_f, gra_f, BLACK)
